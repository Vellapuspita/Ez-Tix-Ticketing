...
export default function DashboardPage() {
  ...
  const handleBuy = (id) => {
    ...
    if (!token) {
      // kalau belum login, kirim ke login dan simpan redirect
...
>
  ...
  <div className="flex-1 p-4">
    <h3 className="text-lg font-semibold text-[#2B2B2B]">{ev.title}</h3>
    ...
    <p className="font-bold mt-2">Rp {ev.price.toLocaleString("id-ID")}</p>
  </div>
    ...
    >
      ...
      <section className="mt-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {events.map((ev) => (
            <div key={ev.id} className="relative rounded-2xl overflow-hidden shadow-md">

  {/* IMAGE */}
  <img src={ev.banner} alt={ev.title} className="w-full h-60 object-cover" />
